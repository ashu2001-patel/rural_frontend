  ...
  return (
    <>
      <style>{`
        ...
        .nav-wrap { position: sticky; top: 0; z-index: 999; display: flex; justify-content: space-between; align-items: center; padding: 0 1.5rem; height: 64px; background: rgba(15,10,5,0.97); backdrop-filter: blur(16px); border-bottom: 1px solid rgba(212,175,99,0.15); font-family: 'Poppins', sans-serif; }
        .nav-glow { position: absolute; bottom: 0; left: 10%; right: 10%; height: 1px; background: linear-gradient(90deg, transparent, rgba(212,175,99,0.4), transparent); }
        ...
        .profile-drawer { position: absolute; top: 64px; right: 1.5rem; width: 260px; background: rgba(16,10,5,0.99); border: 1px solid rgba(212,175,99,0.15); border-radius: 12px; z-index: 1000; overflow: hidden; box-shadow: 0 16px 40px rgba(0,0,0,0.6); animation: drawerIn 0.2s ease; }
        ...
        .nav-mobile { display: none; flex-direction: column; position: absolute; top: 64px; left: 0; right: 0; background: rgba(12,8,4,0.99); backdrop-filter: blur(16px); border-bottom: 1px solid rgba(212,175,99,0.12); z-index: 998; padding-bottom: 0.8rem; }
        ...
        .nm-post { margin: 0.4rem 1.2rem 0.2rem; padding: 10px; background: linear-gradient(135deg, #d4af63, #8b5a2b); border: none; border-radius: 8px; color: #1a0f05; font-weight: 600; font-size: 0.88rem; text-align: center; cursor: pointer; font-family: 'Poppins', sans-serif; text-decoration: none; display: block; }
        ...
        @media (max-width: 900px) { .nav-center { display: none; } .nav-hamburger { display: flex; } .nav-post-btn { display: none; } .profile-drawer { right: 1rem; } }
        @media (max-width: 480px) { .nav-wrap { padding: 0 1rem; } .nav-logo-sub { display: none; } }
      `}</style>
        ...
        {drawerOpen && user && (
          <div className="profile-drawer">
            ...
            <Link to="/my-listings" className="pd-item" onClick={close}>
              <div className="pd-icon amber">📋</div> All My Listings
            </Link>
            ...
            <Link to="/post-tool" className="pd-item" onClick={close}>
              <div className="pd-icon amber">🔧</div> Post Tool
            </Link>
            <Link to="/post-plant" className="pd-item" onClick={close}>
              <div className="pd-icon green">🌱</div> Post Plant
      ...
      <div className={`nav-mobile ${menuOpen ? "open" : ""}`}>
        ...
        {user ? (
          <>
            <div className="nm-section">Post New</div>
            <Link to="/post-animal" className="nm-post" onClick={close}>🐄 Post Animal</Link>
            <Link to="/post-tool" className="nm-post" onClick={close}>🔧 Post Tool</Link>
